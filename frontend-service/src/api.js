...
export const getTickets = () =>
  fetch(`${BASE}/api/tickets`, { headers: headers(true) }).then((r) =>
    r.json(),
  );
...
export const getTicketById = (id) =>
  fetch(`${BASE}/api/tickets/${id}`, { headers: headers(true) }).then((r) =>
    r.json(),
  );

export const getUnassignedTickets = () =>
  fetch(`${BASE}/api/tickets/unassigned`, { headers: headers(true) }).then(
    (r) => r.json(),
  );

// ── SUPPORT SERVICE ───────────────────────────────────────────
export const assignAgent = (ticketId, agentId, customerId, priority) =>
  fetch(`${BASE}/api/support/assign`, {
    method: "POST",
    headers: headers(true),
    body: JSON.stringify({ ticketId, agentId, customerId, priority }),
  }).then((r) => r.json());

export const resolveTicket = (ticketId) =>
  fetch(`${BASE}/api/support/resolve/${ticketId}`, {
    method: "PUT",
    headers: headers(true),
  }).then((r) => r.json());

export const closeTicket = (ticketId) =>
  fetch(`${BASE}/api/support/close/${ticketId}`, {
    method: "PUT",
    headers: headers(true),
  }).then((r) => r.json());

export const addResponse = (ticketId, sender, message) =>
  fetch(`${BASE}/api/support/respond`, {
    method: "POST",
    headers: headers(true),
...
export const markAllRead = (userId) =>
  fetch(`${BASE}/api/notifications/read-all/${userId}`, {
    method: "PATCH",
    headers: headers(),
  }).then((r) => r.json());

export const getUnreadCount = (userId) =>
  fetch(`${BASE}/api/notifications/unread-count/${userId}`, {
    headers: headers(),
  }).then((r) => r.json());
...
export const getTotalTickets = () =>
  fetch(`${BASE}/api/reports/total-tickets`, { headers: headers() }).then((r) =>
    r.json(),
  );
export const getStatusCounts = () =>
  fetch(`${BASE}/api/reports/status-counts`, { headers: headers() }).then((r) =>
    r.json(),
  );
export const getPriorityCounts = () =>
  fetch(`${BASE}/api/reports/priority-counts`, { headers: headers() }).then(
    (r) => r.json(),
  );
export const getAvgResolution = () =>
  fetch(`${BASE}/api/reports/average-resolution`, { headers: headers() }).then(
    (r) => r.json(),
  );
export const getAgentPerformance = () =>
  fetch(`${BASE}/api/reports/agent-performance`, { headers: headers() }).then(
    (r) => r.json(),
  );
