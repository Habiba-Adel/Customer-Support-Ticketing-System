...
//     try {
//       await resolveTicket(ticket._id || ticket.id);
//       onRefresh();               // refresh workspace lists
//       onClose();                 // close modal
//     } catch (err) {
...
//   const customerDisplay = ticket.customerName || ticket.customer || ticket.customerId || 'Guest';
...
//           <div>
//             <h4 className="fw-bold mb-1">Ticket #{ticket.id || ticket._id?.slice(-4)}</h4>
...
//             <span className="fw-bold">{customerDisplay}</span>
//           </div>
//         </div>
...
//           <p className="mb-0 small text-dark">{ticket.description || "No detailed description provided."}</p>
//         </div>
    ...
    <div className={styles.modalOverlay}>
      <div className={styles.modalContent}>
        ...
        <div className="row g-3 mb-4 bg-light p-3 rounded mx-0">
          <div className="col-4">
            <small className="text-muted d-block">Status</small>
            <span className="badge rounded-pill px-3" style={{ backgroundColor: ticket.status === 'Closed' ? 'var(--closed-status)' : 'var(--in-progress-status)' }}>
              {ticket.status}
            </span>
          </div>
          <div className="col-4">
        ...
        {!isReadOnly ? (
          ...
          <div className="pt-3 border-top text-center">
            <p className="text-muted small italic">This ticket is <strong>Closed</strong>. Response is disabled.</p>
          </div>
        )}
      </div>
    </div>
